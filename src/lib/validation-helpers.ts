...
export function _validateObject(object: Record<string, any>) {
  ...
  Object.entries(object).forEach(([dictKey, value]) => {
    let currentProperty = Object.keys(types).find(
      (key) => key == dictKey
      ...
      if (_isValidProperty(value, types[currentProperty])) {
        switch (typeof value) {
          case "object":
            ...
            let is3DProperty = Object.keys(arrayProperties).find(
              (key) => key == dictKey
            ) as keyof typeof arrayProperties;
            if (is3DProperty) {
              // we have position,scale or rotation or another array
              let isValid = _isValidArray(
                object[currentProperty],
                arrayProperties[is3DProperty].length,
                "number"
              );
              if (isValid) {
                ...
                console.error(
                  `[Scripting]${currentProperty} should be of type array of length 3`
                );
              }
            } else {
              // it's just an object;
        ...
        if (is3DProperty) {
          ...
          console.error(
            `[Scripting] ${currentProperty} should be of type ${types[currentProperty]}`
          );
        }
      }
    } else {
