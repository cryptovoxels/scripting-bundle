...
import {
  ...
  Color3,
  Matrix,
} from "./vendor/babylonjs.js";
import { Animation } from "./vendor/babylonjs.js";
...
//  if(msg.type=='click'){console.log('onMessage', msg)}
  ...
  join(player) {
    ...
    let p = this.players.find((p)=>p.wallet === player.wallet && p.uuid === player.uuid)
    if(p){
      return
    }
    this.emit('playerenter', {player:player instanceof Player?player:new Player(player,this)});
  ...
  leave(player) {
    ...
    this.emit('playerleave', {player:player instanceof Player?player:new Player(player,this)});
  }
  ...
  parse(parcel) {
    ...
    this.featuresList = Array.from(parcel.features).map(f => !!f&&Feature.create(this, f));
  ...
  self.onmessage = e => {
    ...
    let oldPlayer = this.players.find(p=>p.wallet == e.data.player.wallet)
    if (oldPlayer){
      // we have an old player (perfect)
      ws.player = oldPlayer instanceof Player? oldPlayer : new Player(oldPlayer,this)
    }
